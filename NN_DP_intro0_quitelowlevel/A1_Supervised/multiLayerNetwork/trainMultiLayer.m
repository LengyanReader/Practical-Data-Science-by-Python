...
for n = 1:numIterations
    ...
    %------------------------------------------------
    grad_v = (YTrain'-DTrain')*HTrain'.*(2/NTrain); % Gradient for the output layer

    Vout_ = Vout; % remove bias weight in V
    Vout_(:,end) =[];
    HTrain_=HTrain; % remove bias in activation
    HTrain_(end,:)=[];
    
    grad_w = (Vout_'*(YTrain'-DTrain').*(1-HTrain_.*HTrain_))*XTrain.*(2/NTrain); % And the input layer
   %--------------------------------------------------------------------- 
